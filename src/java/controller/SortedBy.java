 ...
 * @author Asus
 */
@WebServlet(name = "SortedBy", urlPatterns = {"/sortedby"})
public class SortedBy extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SortedBy</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SortedBy at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String sorted_raw = request.getParameter("sorted");
        int categoryId, page, sorted;
        ...
        try {
            ...
            sorted = Integer.parseInt(sorted_raw);

            // get type and firm of product
            type = dao.displayType(jsp_raw, categoryId);
            firm = dao.displayFirm(jsp_raw, categoryId);
            
            // get all product by sorted
            if(sorted == 1){
                list = dao.fearturedProduct(categoryId);
            }else if(sorted == 2){
                list = dao.lthProduct(categoryId);
            }else if(sorted == 3){
                list = dao.htlProduct(categoryId);
            }
            ...
            request.setAttribute("urlpattern", "sortedby");
