...
public class CommentProduct extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CommentProduct</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CommentProduct at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (comment.trim().replaceAll("\\s+", "").length() == 0) {
                response.sendRedirect("detail-product?pid=" + pid + "&cateId=" + catedId);
            } else {
                dao.insertComment(pid, uid, comment, "");
