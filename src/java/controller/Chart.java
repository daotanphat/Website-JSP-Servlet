...
public class Chart extends HttpServlet {
    ...
    throws ServletException, IOException {
        Map<String, Float> numSell = new HashMap<>();
        Map<String, Integer> topSeller = new HashMap<>();
        DAO dao = new DAO();
        int sum = 0;
        for(Map.Entry<String, Integer> entry: dao.chart1().entrySet()){
            sum += entry.getValue();
        }
        
        for(Map.Entry<String, Integer> entry: dao.chart1().entrySet()){
            numSell.put(entry.getKey(), (float)entry.getValue() / sum * 100);
        }
        
        topSeller = dao.fiveSellest();
        request.setAttribute("numSell", numSell);
        request.setAttribute("topSeller", topSeller);
        
        int numComment = dao.numComment();
        int numUser = dao.numUser();
        int numOrder = dao.numOrder();
